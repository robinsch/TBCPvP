...
bool Pet::LoadPetFromDB(Player* owner, uint32 petentry, uint32 petnumber, bool current)
{
    ...
    if (owner->getClass() == CLASS_HUNTER || owner->getClass() == CLASS_WARLOCK) // remove unnecessary use of PQuery
    {
        if (petnumber)
            ...
            if (m_owner->GetTypeId() == TYPEID_PLAYER)
            {
                switch (m_owner->getClass())
                {
                    case CLASS_WARLOCK:
                    {
                        ...
                        if (GetEntry() == 17252)
                        {
                            SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float((petlevel*2.15) - (petlevel / 4)));
                            SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float((petlevel*2.74) + (petlevel / 4)));
                        }
                        else
                        {
