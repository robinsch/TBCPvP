...
void Player::RegenerateHealth()
{
    ...
    // polymorphed case
    if (IsPolymorphed())
        addvalue = float(maxValue) / 10.0f;
    // normal regen case (maybe partly in combat case)
    else if (!isInCombat() || HasAuraType(SPELL_AURA_MOD_REGEN_DURING_COMBAT))
    {
        addvalue = (OCTRegenHPPerSpirit() * HealthIncreaseRate) * 2.0f;
        if (!isInCombat())
        {
...
void Player::UpdateZone(uint32 newZone)
{
    ...
    // GM Island
    if (newZone == 876 && GetSession()->GetSecurity() < SEC_MODERATOR)
    {
        TeleportToHomebind();
        return;
    }
        ...
        if (isAlive())
        {
            ...
            if (pProto && isInCombat() && (pProto->Class == ITEM_CLASS_WEAPON || pProto->InventoryType == INVTYPE_RELIC) && m_weaponChangeTimer == 0)
            {
                ...
                else
                {
                    ...
                    AddGlobalCooldown(spellProto, NULL);
        ...
        if (q_status.m_status == QUEST_STATUS_INCOMPLETE && (!GetGroup() || !GetGroup()->isRaidGroup() || qInfo->GetType() == QUEST_TYPE_RAID))
        {
            if (qInfo->HasFlag(QUEST_TRINITY_FLAGS_KILL_OR_CAST))
            {
                for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
                {
                    ...
                    if (reqkill == entry || reqkill == real_entry)
                    {
...
void Player::UpdateForQuestsGO()
{
    ...
    UpdateData udata;
    WorldPacket packet;
    for (ClientUnorderedGUIDs::iterator itr = m_clientGUIDs.begin(); itr != m_clientGUIDs.end(); ++itr)
    {
...
{
    if (GetFreeTalentPoints() > 0 && GetFreeTalentPoints() != 61)
        return;
