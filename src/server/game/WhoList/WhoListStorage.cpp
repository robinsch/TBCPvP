...
void WhoListStorageMgr::Update()
{
    ...
    _whoListStorage.reserve(sWorld->GetMaxActiveSessionCount() - sWorld->GetMaxQueuedSessionCount() + 1);
    ...
    for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
    {
        ...
        _whoListStorage.emplace_back(itr->second->GetGUID(), itr->second->GetTeam(), AccountTypes(itr->second->GetSession()->GetSecurity()), itr->second->getLevel(), 
            itr->second->getClass(), itr->second->getRace(), itr->second->GetZoneId(), itr->second->GetByteValue(PLAYER_BYTES_3, 0), itr->second->IsVisible(), 
