...
void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket & recv_data)
{
    ...
    else if (IS_PLAYER_GUID(guid))
    {
        ...
        {
            sLog->outDebug("WORLD: HandleGossipSelectOptionOpcode - Player (GUID: %u) not found.", uint32(GUID_LOPART(guid)));
            return;
        }
    }
    else
    ...
    else
    {
        if (unit)
        {
            ...
            sScriptMgr->ItemGossipSelect(_player, item, _player->PlayerTalkClass->GossipOptionSender(gossipListId), _player->PlayerTalkClass->GossipOptionAction(gossipListId));
        else
            sScriptMgr->OnGossipSelect(_player, menuId, _player->PlayerTalkClass->GossipOptionSender(gossipListId), _player->PlayerTalkClass->GossipOptionAction(gossipListId));
    }
}
...
void WorldSession::HandleWhoOpcode(WorldPacket& recvData)
{
    ...
    // client send in case not set max level value 70 but Trinity supports 255 max level,
    // update it to show GMs with characters after 70 level
    ...
    for (WhoListPlayerInfo const& target : whoList)
    {
            ...
            if (target.GetSecurity() > AccountTypes(gmLevelInWhoList) && !gmLevelInWhoList)
                continue;

            // check if target is globally visible for player
            if (_player->GetGUID() != target.GetGuid().GetRawValue() && !target.IsVisible())
                if (target.GetSecurity() > _player->GetSession()->GetSecurity())
                    continue;
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket & recv_data)
{
    ...
    if (sObjectMgr->IsTavernAreaTrigger(Trigger_ID))
    {
        ...
        GetPlayer()->InnEnter(time(NULL), atEntry->mapid, atEntry->x, atEntry->y, atEntry->z, atEntry->radius);
